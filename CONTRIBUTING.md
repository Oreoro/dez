...
The Dez culture values working code and synchronous conversations over long
discussion threads.
...
- Make sure the change is **desired**: we're always happy to accept bugfixes,
  ...
  - This especially applies to any changes proposed to the Dez Extension API.
- Include a clear description of **what you're solving**, and why it's important.
- Include **tests**. For UI changes, consider updating visual regression tests (see [Building Dez for macOS](./docs/src/development/macos.md#visual-regression-tests)).
...
- Does it match Dez’s design language (spacing, typography, icons)?
- Are terminology, labels, and tone consistent with the rest of Dez?
...
- Do we avoid internal Dez jargon that only insiders would know?
...
- Are error messages actionable and consistent with Dez’s voice?
...
## Bird's-eye view of Dez

We suggest you keep the [Dez glossary](docs/src/development/glossary.md) at your side when starting out. It lists and explains some of the structures and terms you will see throughout the codebase.

Dez is made up of several smaller crates - let's go over those you're most likely to interact with:

- [`gpui`](/crates/gpui) is a GPU-accelerated UI framework which provides all of the building blocks for Dez. **We recommend familiarizing yourself with the root level GPUI documentation.**
- [`editor`](/crates/editor) contains the core `Editor` type that drives both the code editor and all various input fields within Dez. It also handles a display layer for LSP features such as Inlay Hints or code completions.
- [`project`](/crates/project) manages files and navigation within the filetree. It is also Dez's side of communication with LSP.
- [`workspace`](/crates/workspace) handles local state serialization and groups projects together.
- [`vim`](/crates/vim) is a thin implementation of Vim workflow over `editor`.
- [`lsp`](/crates/lsp) handles communication with external LSP server.
- [`language`](/crates/language) drives `editor`'s understanding of language - from providing a list of symbols to the syntax map.
- [`rpc`](/crates/rpc) defines messages exchanged with Dez's SSH remote server.
