...
- [x] 3.5 Add bundle job for macOS aarch64 on `macos-15` runner producing `dez-aarch64.dmg`
- [x] 3.6 Add bundle job for macOS x86_64 on `macos-13` runner producing `dez-x86_64.dmg`
- [x] 3.7 Add bundle job for Linux aarch64 on `ubuntu-24.04-arm` runner producing `dez-linux-aarch64.tar.gz`
- [x] 3.8 Add bundle job for Linux x86_64 on `ubuntu-24.04` runner producing `dez-linux-x86_64.tar.gz`
- [x] 3.9 Add bundle job for Windows x86_64 on `windows-2022` runner producing `dez-x86_64.exe`
- [x] 3.10 Add bundle job for Windows aarch64 on `windows-11-arm` runner producing `dez-aarch64.exe`
...
- [x] 5.1 Update `script/determine-release-channel` to work without `dez-industries` specific configuration
- [x] 5.2 Verify `script/generate-licenses` and `script/generate-licenses.ps1` work without external dependencies
- [x] 5.3 Verify `script/create-draft-release` works with the current repository (not hardcoded to `dez-industries/dez`)
