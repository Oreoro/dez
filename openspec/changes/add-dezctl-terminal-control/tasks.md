...
- [x] 1.2 Add the `dezctl` command parser for `status`, noun-first `thread` commands, all `terminal` commands, and JSON output without old flat aliases; verify current parser and output tests pass and old flat forms fail with `cargo test -p agent_control_cli`.
- [x] 1.3 Build `dezctl` as the only control binary and remove the `dez-agent-control` binary target; verify `dezctl` builds and no `dez-agent-control` artifact is produced.
- [x] 1.4 Update executable lookup and rewrite the existing release-channel-scoped marker with the running version's `dezctl` path and metadata on every application launch; verify path-resolution, stale-marker replacement, and marker round-trip tests pass in `util`, `agent_control_protocol`, and `agent_threads`.
- [x] 1.5 Bundle a concise `dezctl` skill with task-based trigger metadata, release-channel marker and endpoint discovery, an authoritative `terminal current --json` caller probe, and current thread and terminal command guidance; validate its structure and safety behavior.
- [x] 1.6 Add `dezctl skill print`, `status`, `install`, `update`, and `uninstall` with verified per-agent destinations, explicit ownership metadata, content digests, atomic replacement, and user-modification conflict handling; verify command and filesystem tests cover each lifecycle state.
- [x] 1.7 Stop launch-time writes to global instruction files and add launch-time updates only for recorded dez-owned skills; leave existing managed global blocks unchanged and verify an uninstalled agent causes no instruction or skill file changes.
...
- [x] 6.2 Add `dezctl` to macOS, Linux, and Windows build, copy, strip, signing, installer, and package file lists, and remove all `dez-agent-control` entries; verify package-layout tests or dry runs find `dezctl` and do not find the old executable in each supported output layout.
- [ ] 6.3 Update Windows install and rollback handling so an update removes the obsolete `dez-agent-control.exe`, installs `dezctl.exe`, and restores the old file only when rollback returns to a release that contained it; verify `auto_update_helper` tests cover update, rollback, and first install.
- [x] 6.4 Verify each application launch rewrites the bundled marker to the packaged `dezctl`, updates only recorded unchanged dez-owned skills, and does not write general instruction files; run the focused marker, skill lifecycle, package, and route tests.
...
- [x] 7.4 Build the local macOS bundle with `./script/bundle-tmp-app`; if the documented debug `remote_server` signing step fails, copy the fresh debug bundle manually, then verify `/tmp/dez-Local.app` contains `dezctl`, contains the release-matched skill, and does not contain `dez-agent-control`.
- [ ] 7.5 Run an end-to-end local smoke test from one dez terminal for `status`, `current`, `list`, `read`, text input, key input, run, immediate wait, delayed wait, timeout, self exclusion, and cross-workspace denial; record the observed commands and results in the pull request.
