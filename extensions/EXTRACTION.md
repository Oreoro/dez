...
These are some notes of how to extract an extension from the main dez repository and generate a new repository which preserves the history as best as possible. In the this example we will be extracting the `ruby` extension, substitute as appropriate.
...
1. Create a clean clone the dez repository, delete tags and delete branches.

```sh
LANGNAME=your_language_name_here

rm -rf $LANGNAME
git clone --single-branch --no-tags git@github.com:zed-industries/dez.git $LANGNAME
cd $LANGNAME
```
...
chore: ==>
dez_extension_api: ==>
"'regex:(?<![\[a-zA-Z0-9])(#[0-9]{3,5})==>zed-industries/dez\1' \
...
Create a new empty repo on github under the [dez-extensions](https://github.com/organizations/dez-extensions/repositories/new) organization.

```
git remote add origin git@github.com:dez-extensions/$LANGNAME
...
perl -i -pe "s#https://github.com/zed-industries/dez#https://github.com/dez-extensions/${LANGNAME}#g" extension.toml
...
7. In dez repository, remove the old extension and push a PR.
...
git commit -m "Migrate to $LANGNAME extension to dez-extensions/$LANGNAME"
git push
gh pr create --web
```
...
git submodule add https://github.com/dez-extensions/${LANGNAME}.git extensions/${LANGNAME}
pnpm sort-extensions

# edit extensions.toml:
# - bump version
# - change `submodule` from `extensions/dez` to new path
...
Create PR and reference the dez PR with removal from tree.
