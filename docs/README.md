...
Welcome to dez's documentation.

This is built on push to `main` and published automatically to [https://dez.dev/docs](https://dez.dev/docs).
...
If for some reason you need to bypass the docs preprocessor, you can comment out `[preprocessor.dez_docs_preprocessor]` from the `book.toml`.

## Images and videos

To add images or videos to the docs, upload them to another location (e.g., dez.dev, GitHub's asset storage) and then link out to them from the docs.

Putting binary assets such as images in the Git repository will bloat the repository size over time.

## Internal notes:

- We have a Cloudflare router called `docs-proxy` that intercepts requests to `dez.dev/docs` and forwards them to the "docs" Cloudflare Pages project.
...
{#kb scope::Action} - e.g., {#kb dez::OpenSettings}.
...
{#action scope::Action} - e.g., {#action dez::OpenSettings}.

This will render a human-readable version of the action name, e.g., "dez: open settings", and will allow us to implement things like additional context on hover, etc.
