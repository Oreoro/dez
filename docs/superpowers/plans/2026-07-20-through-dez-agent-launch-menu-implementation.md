# Through-dez Agent Launch and Menu Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Make every Codex and Claude new/resume action use the pinned managed CLI in Through-dez mode while reducing the dropdown to ordinary launch options and remote sign-out.
...
**Tech Stack:** Rust, GPUI tasks and context menus, dez remote-agent routing, managed-agent provisioning, Cargo tests.

## Global Constraints

- Change Through-dez behavior only.
- Keep Not-through-dez menu, ambient launch, explicit managed row, and resume behavior unchanged.
- Apply the Through-dez rule equally to Codex and Claude.
- Every new-thread entry point, resume option, and automatic restoration must use the pinned managed executable through dez.
- Keep provisioning status in the existing popup notification; do not render a separate managed/status row in Through-dez mode.
...
### Task 1: Shared Through-dez route policy and managed resume
...
Expected: FAIL for Claude in Through-dez mode because `uses_managed_resume` is Codex-only.
...
git commit -m "Route Claude resume through dez"
```
...
Do not alter the explicit managed function's Not-through-dez routing in this
task.
...
- Produces: ordinary `launch_new_thread` dispatch that selects managed preparation only for Through dez, plus `launch_managed_thread_for_route(..., required_route: Option<RequiredAgentRoute>)` for shared orchestration.
...
Assert `ManagedTunneled` only for Through dez and `Configured` otherwise.
...
Keep public `launch_managed_thread` as a Not-through-dez-compatible wrapper
...
```bash
git add crates/agent_threads/src/store.rs
git commit -m "Route new agent threads through dez"
```

---

### Task 4: Through-dez dropdown simplification
...
- Produces: `show_explicit_managed_launch(managed_available: bool, tunneled: bool) -> bool`, used to render the explicit managed row only outside Through-dez mode.
...
let managed_label = show_explicit_managed_launch(managed_available, tunneled)
    .then(|| {
        workspace.upgrade().map_or_else(
            || SharedString::from(format!("New — dez-managed {}", kind.label)),
...
git commit -m "Simplify Through-dez agent menus"
```
...
**Files:**
- Modify: `docs/superpowers/specs/2026-07-20-through-dez-agent-launch-menu-design.md`
- Modify: `docs/superpowers/plans/2026-07-20-through-dez-agent-launch-menu-implementation.md`

**Interfaces:**
- Consumes: completed route-aware new/resume behavior and menu policy.
- Produces: an automated verification record, explicit live-Claude deferral, and a fresh `/tmp/dez-Local.app` for manual Codex validation.
...
fresh bundle is created, preserve the old `/tmp/dez-Local.app`, copy
`target/aarch64-apple-darwin/debug/bundle/osx/dez.app` into its place, compare
the source and copied `Contents/MacOS/dez` SHA-256 digests, and run
...
validation remains deferred and that Not-through-dez was protected by unit
policy tests rather than changed.

- [x] **Step 4: Commit the verification record**

```bash
git add docs/superpowers/specs/2026-07-20-through-dez-agent-launch-menu-design.md docs/superpowers/plans/2026-07-20-through-dez-agent-launch-menu-implementation.md
git commit -m "Document Through-dez launch verification"
```
