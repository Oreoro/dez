# Claude Platform Through-dez Egress Fix Design
...
The pinned Claude CLI connects to `platform.claude.com`, but dez's Claude
Through-dez destination policy still permits the obsolete
`console.anthropic.com` hostname. dez's CONNECT proxy therefore returns
...
The active dez egress design already requires `platform.claude.com` instead
...
test, the complete `agent_threads` suite, formatting, and dez clippy.

Build a fresh `/tmp/dez-Local.app` for live validation on the user's Claude
remote. The user performs the live provider request; automated tests do not
contact Anthropic.

## Acceptance Criteria

- Through-dez Claude permits CONNECT to `platform.claude.com:443`.
...
- Codex and Not-through-dez behavior remain unchanged.
...
live Through-dez Claude request on the remote host.
