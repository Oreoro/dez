# `dezctl` for Remote Development

## Status

This document is a design proposal. Local `dezctl` terminal control is
implemented. Remote terminal control is not implemented.

## Goal

Let a process in a dez remote-development terminal use the same `dezctl`
commands as a process in a local terminal. Keep terminal state and control in
the local dez application. Use the existing authenticated connection to
`dez-remote-server` as the bridge. Do not open a public network port.
...
dezctl thread retie --worktree <path> [--json]
dezctl thread create --worktree <current|new> [--name <name>] --agent <agent> --prompt <prompt> [--split <left|right|up|down>] [--focus] [--json]

dezctl terminal current [--json]
dezctl terminal list [--all] [--json]
dezctl terminal open [--cwd <path>] [--focus] [--json]
dezctl terminal split (--current|--terminal <terminal-id>) --direction <left|right|up|down> [--cwd <path>] [--focus] [--json]
dezctl terminal read <terminal-id> [--source visible|recent|recent-unwrapped|detection] [--lines <count>] [--since <cursor>] [--json]
dezctl terminal send-text <terminal-id> <text> [--json]
dezctl terminal send-key <terminal-id> <key>... [--json]
dezctl terminal run <terminal-id> <command> [--json]
dezctl terminal wait-output <terminal-id> (--match <text>|--regex <pattern>) [--source visible|recent|recent-unwrapped|detection] [--lines <count>] [--timeout <duration>] [--json]
```
...
dez displays a remote terminal through a local `Terminal` entity. The remote
shell and coding agent run in a PTY managed by `dez-remote-server`. The local
`Terminal` owns the emulated screen, scrollback, input path, and dez terminal
ID.

Local `dezctl` connects to a release-channel-scoped Unix socket or Windows
named pipe. dez gets the peer process ID from the operating system and first
...
root. dez rejects remote terminals because the remote process cannot connect
...
Use `dez-remote-server` as a remote `dezctl` bridge. Install a sibling
command named `dezctl` that starts the same binary in client mode. The client
...
server-created caller identity to local dez. Local dez validates the
...
`dez-remote-server` binary with a `dezctl` client mode. Dispatch this mode
from the executable file name so the same binary supports both command parsers.

After a remote server version is installed:

- On Unix, create a sibling link named `dezctl` to the versioned server.
- On Windows, create a sibling `dezctl.exe` launcher or copy.
- Write a release-channel- and version-scoped executable marker under dez's
  managed remote data directory.
- Replace each supported dez-managed remote agent instruction block with the
  instructions from this dez version.
...
The parser and control protocol come from the same dez installation. A new
...
When `dez-remote-server` creates a PTY, it creates an opaque remote terminal
registration ID and records the PTY root process identity. The ID is unique
for the life of that server process. The server sends it to local dez in the
remote terminal creation flow. Local dez stores this caller data with the
...
When remote `dezctl` connects, the server gets its peer process ID from the
...
a match, it forwards the server-side registration ID. Local dez accepts that
...
returns `not-ready`, and `dezctl` uses the current bounded retry behavior.
Agent kind and session metadata use the same connection and registration
generation. Local dez sends an update when session discovery attaches an ID.
...
session variable from the true peer process. Local dez sends the Agent
...
Fresh Codex threads have no dez-assigned session ID. Extend the existing
...
Local dez still verifies that the registration is live, belongs to the
...
Local dez rejects a remote request when:
...
## Endpoint discovery and multiple dez instances

One remote host can have multiple server processes for different dez
...
Remote `dezctl` reads the matching directory and tries its bounded set of
...
Thus, operating-system ancestry selects the matching dez instance without
...
Local dez must not accept an envelope from a connection that did not create
the registration.

The server does not resolve local terminal IDs, read terminal content,
interpret operations, or write terminal input. Local dez uses the current
...
disconnect, project disconnect, dez shutdown, and terminal release cancel an
...
The server owns its PTY process-to-registration map. Local dez owns the
...
- Tunneled uses only the pinned dez-managed executable on the remote host and
  routes its network traffic through local dez.
...
For a remote workspace, local dez performs pane placement and asks the
existing project terminal service to create the PTY through
`dez-remote-server`. The
...
returns it through the existing terminal creation flow. Local dez waits for
the matching local registry entry before it completes the control request.

If remote PTY creation or registration fails, local dez removes any partial
...
only the pinned dez-managed executable and its existing local traffic
...
- `status --json` reports the local dez version, protocol version, release
  channel, and supported capabilities.
- `terminal current` returns the caller's dez terminal metadata.
...
If no server can verify the caller, `dezctl` reports that the process is not
in a controllable dez remote terminal. If a server verifies the caller but
has no local dez connection, it reports that the matching session is
unavailable. It does not start dez or another server.
...
not attach silently to a different dez instance.
...
- `remote-control-unavailable` when the matching bridge has no local dez
  connection;
- `remote-session-stale` when the server resolves a caller but local dez no
...
- allow control across local workspaces or dez instances;
...
- keep terminals alive after local dez exits.
