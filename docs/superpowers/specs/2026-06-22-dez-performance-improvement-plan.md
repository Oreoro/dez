# dez performance improvement plan

## Summary

Improve dez's CPU and memory behavior by reducing fork-specific always-on
...
narrows the dez-specific startup surface but does not by itself prove a CPU or
memory improvement:
...
- Current `crates/dez/src/dez.rs` startup wiring does not load an inherited
  assistant panel, while it does eagerly load Agent Threads.

Agent Threads is therefore the strongest currently identified dez-specific
...
- Reduce dez-specific startup CPU and memory without removing Agent Threads.
...
- Keep changes localized to Agent Threads and dez workspace panel wiring,
...
- No attempt to make dez outperform Zed across inherited baseline systems in
  one pass.
...
- The existing `dez_actions::agent_threads::ToggleFocus` handler (added in
...
Lazy loading and active-state gating should reduce dez-specific startup work
