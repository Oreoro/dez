...
sign-out, and provider-management actions for Codex and Claude. A dez-managed
...
cannot prove that the stored credential is accepted or that Through-dez
...
dez's managed installation, so they fail on a remote host where only the
dez-managed binary exists.
...
- When the project route is **Through dez**, sign-out must use the pinned
  dez-managed executable for that agent and retain the Through-dez route
  guarantee.
...
Users who need to authenticate select **New — dez-managed Codex** or
**New — dez-managed Claude**. If the remote credential is absent, the
...
so sign-out follows the selected remote-agent route. Under Through dez,
...
self-update suppression policy, and launch it with Through dez as the
...
- Through-dez sign-out replaces only the executable with the corresponding
  managed path, retains the logout arguments and configured environment,
  applies self-update suppression, and requires the Through-dez route; and
- Not-through-dez sign-out uses the configured command without managed
  provisioning.

The automated tests do not launch either provider CLI or contact its service.
The full `agent_threads` suite, formatting check, and dez clippy check run
...
- Through-dez sign-out runs the corresponding pinned managed executable
  through dez, even if the remote host has its own agent installation or
  internet access.
- Not-through-dez sign-out uses the configured command for the selected
  agent.
- Launching dez-managed Codex remains unchanged.
- Launching dez-managed Claude remains unchanged.
...
sign-out for Codex and Claude. Through-dez sign-out prepares the pinned
managed executable and requires the Through-dez route; Not-through-dez
