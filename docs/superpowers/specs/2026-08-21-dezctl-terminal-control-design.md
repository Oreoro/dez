...
Replace the long `dez-agent-control` command name with `dezctl` and make
the command a stable local control surface for dez. Keep the existing Agent
Thread operations and add a small set of commands that let a process in one
dez terminal inspect, operate, and create another terminal in the same
...
dezctl thread retie --worktree <path>
dezctl thread create --worktree <current|new> --agent <agent> --prompt <prompt> [--split <left|right|up|down>] [--focus]

dezctl terminal current
dezctl terminal list
dezctl terminal open [--cwd <path>] [--focus]
dezctl terminal split (--current|--terminal <terminal-id>) --direction <left|right|up|down> [--cwd <path>] [--focus]
dezctl terminal read <terminal-id> [--source visible|recent|recent-unwrapped] [--lines <count>]
dezctl terminal send-text <terminal-id> <text>
dezctl terminal send-key <terminal-id> <key>...
dezctl terminal run <terminal-id> <command>
dezctl terminal wait-output <terminal-id> (--match <text>|--regex <pattern>) [--timeout <duration>]
```

`dezctl` controls a running dez application. It is not a general terminal
multiplexer and does not keep terminal processes alive after dez exits.

## Current state

`dez-agent-control` currently has two flat commands:
...
process that dez tracks as a live Agent Thread.

dez already has the terminal primitives that the new commands need:
...
multiplexer, while dez is an editor that owns terminal entities for the life
...
- Make the CLI the normal automation surface. Keep the socket protocol as an
  implementation interface until dez has a reason to support third-party
  protocol clients.
...
operation through dez's existing pane model.

## Command name and instruction upgrades

The installed executable becomes `dezctl`. Command names use noun-first
groups so that later additions do not create another flat list:

```text
dez-agent-control retie-thread ...  -> dezctl thread retie ...
dez-agent-control create-thread ... -> dezctl thread create ...
```

The application bundle and packages contain only `dezctl`. The command does
...
name, but every dez launch rewrites its JSON value to the running version's
`dezctl`. The same launch synchronizes a versioned dez-managed block in
...
larger. Module and help text use the new `dezctl` name where they describe
...
Add a process-local `TerminalControlRegistry` owned by the dez application.
...
process IDs, or GPUI entity IDs. They are not reused during one dez process.
...
Terminal commands must also work from an ordinary dez terminal. Therefore,
...
unique cwd match would weaken the rule that only a process in a dez terminal
...
- A terminal command is accepted through a strong live local dez PTY
...
- An ordinary process outside a dez terminal is rejected. The Agent Thread
...
For `--worktree new`, dez launches the Agent Thread in the destination
...
`dezctl status --json` command reports the running dez version, protocol
...
whose registry entry has not completed. `dezctl` applies the existing
...
`dez-remote-server` bridge described in
`docs/superpowers/specs/2026-08-22-dezctl-remote-dev-design.md`.
...
terminal to select the workspace, exact pane, and split position. Local dez
...
workspace. dez does not support a local terminal in a remote workspace.
...
executable. Tunneled uses only the pinned dez-managed executable and its
existing local traffic tunnel.

## Packaging and discovery

Build and package `dezctl` beside the current helper location on every
supported platform. Update Agent Thread instructions to discover the executable
through the existing marker. Do not require `dezctl` to be in `PATH`.

`dezctl` does not locate and start dez. If no matching control endpoint
exists, it reports that dez is not running or that the release channel does
not match.

Release-channel scoping remains part of the endpoint and marker names. A Stable
client must not connect to a Nightly or development dez process by accident.
...
Package tests verify that `dezctl` is in the macOS application bundle and
Linux and Windows packages and that `dez-agent-control` is absent.
Instruction tests verify that every new dez launch installs the current
`dezctl` discovery text.

## Non-goals

The first version does not:

- keep terminals alive after dez exits;
...
- guarantee terminal IDs across dez restarts.
...
1. Add the `dezctl` command hierarchy, protocol version and status result,
