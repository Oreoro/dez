...
`session_rail.visibility = "hidden"` removes the rail from layout. Compact,
overlay, always, and auto modes share the current rail surface until distinct
render modes are added.

`session_rail.mode = "icon"` narrows the rail and hides row labels/metadata,
while `session_rail.mode = "detailed"` keeps a wider minimum width. The first
