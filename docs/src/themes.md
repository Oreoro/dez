---
title: Themes - dez
description: Browse, install, and create themes for dez. Includes built-in themes and community theme extensions.
---

# Themes

dez comes with a number of built-in themes, with more themes available as extensions.
...
You can find hundreds of different theme options in dez's extensions store, which you can access via the command palette with {#action dez::Extensions} or the [dez website](https://dez.dev/extensions?filter=themes).

Many popular themes have been ported to dez, and if you're struggling to choose one, visit [dez-themes.com](https://dez-themes.com), a third-party gallery with visible previews for many of them.

## Build Your Theme

You can use [dez's Theme Builder](https://dez.dev/theme-builder) to design your own custom theme based on an existing one.

This tool lets you fine-tune and preview how every surface in the dez app will look.
You can then export the JSON for [local use](./themes.md#local-themes) or for [publishing in dez's extension store](./extensions/themes.md).

## Configuring a Theme

Your selected theme is stored in your settings file.
You can open your settings file from the command palette with {#action dez::OpenSettingsFile} (bound to {#kb dez::OpenSettingsFile}).

By default, dez maintains two themes: one for light mode and one for dark mode.
...
For example, [assets/themes/one/one.json](https://github.com/zed-industries/dez/blob/main/assets/themes/one/one.json) for the default One Dark and One Light themes.

## Local Themes {#local-themes}

Store new themes locally by placing them in the `~/.config/dez/themes` directory (macOS and Linux) or `%USERPROFILE%\AppData\Roaming\dez\themes\` (Windows).

For example, to create a new theme called `my-cool-theme`, create a file called `my-cool-theme.json` in that directory.
It will be available in the theme selector the next time dez loads.
