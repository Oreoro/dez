...
/// non-zero exit status even though `dez --printenv` ran successfully and
...
async fn capture_unix(
    ...
    let quoted_dez_path = super::get_shell_safe_dez_path(shell_kind)?;
    ...
    // See: https://github.com/zed-industries/dez/pull/32136#issuecomment-2999645482
    ...
    command_string.push_str(&format!("{} --printenv {}", quoted_dez_path, redir));
...
async fn capture_windows(
    ...
    let dez_path_string = dez_path.display().to_string();
    ...
    let quoted_dez_path = quote_for_shell(&dez_path_string)?;
    let cmd = match shell_kind {
        ...
        | ShellKind::Posix => cmd.args([
            ...
            "-i",
            "-c",
            &format!("cd {}; {} --printenv", quoted_directory, quoted_dez_path),
        ]),
        ShellKind::PowerShell | ShellKind::Pwsh => cmd.args([
            ...
            &format!(
                ...
                quoted_directory, quoted_dez_path
            ),
        ]),
        ...
        ShellKind::Nushell => {
            let dez_command = shell_kind
                .prepend_command_prefix(&quoted_dez_path)
                .into_owned();
            cmd.args([
                "-c",
                &format!("cd {}; {} --printenv", quoted_directory, dez_command),
            ])
        }
        ShellKind::Cmd => {
