...
const ICON_DIR: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/../dez/resources/windows");
...
pub fn compile(manifest: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (icon_filename, product_name) = match channel {
        "stable" => ("app-icon.ico", "dez"),
        "preview" => ("app-icon-preview.ico", "dez Preview"),
        "nightly" => ("app-icon-nightly.ico", "dez Nightly"),
        _ => ("app-icon-dev.ico", "dez Dev"),
    };
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            VALUE "CompanyName", "dez Industries, Inc.\0"
            VALUE "LegalCopyright", "Copyright 2022 - 2025 dez Industries, Inc.\0"
        END
    END
    BLOCK "VarFileInfo"
    BEGIN
...
"#
    ...
    let rc_path = out_dir.join("dez_resources.rc");
