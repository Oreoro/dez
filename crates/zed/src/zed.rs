...
fn restore_dez_visual_profile(settings: &mut settings::SettingsContent) {
    let ui_font = settings::FontFamilyName(Arc::from("IBM Plex Sans"));
    let code_font = settings::FontFamilyName(Arc::from("Lilex"));
...
mod tests {
    ...
    use fs::FakeFs;
    use gpui::{
        Action, AnyWindowHandle, App, BorrowAppContext, Modifiers, OwnedMenuItem, TestAppContext,
    ...
    fn dez_visual_profile_restores_identity_without_resetting_font_size() {
        ...
        assert_eq!(settings["ui_font_family"], "IBM Plex Sans");
        assert_eq!(settings["buffer_font_family"], "Lilex");
        assert_eq!(settings["markdown_preview_code_font_family"], "Lilex");
        assert_eq!(
            settings["theme"],
            json!({
                "mode": "system",
    ...
    fn dez_drops_inherited_collaboration_keybindings() {
        ...
        // The Collaboration panel is restored as a Workspace Tool in Dez, so
        // its panel-level keybindings stay available.
        assert!(!is_dez_hidden_keybinding_name(
