    ...
    fn settings_view(&self, _cx: &mut App) -> Option<ProviderSettingsView> {
        ...
        Some(ProviderSettingsView::SubPage(
            ...
            .description(InlineDescription::Text(
                "To use the Dez agent with Bedrock, set a custom authentication strategy in your settings or use static credentials. Mantle-only models (for example GPT-5.5, GPT-5.4, and Grok 4.3) also require IAM permissions for the `bedrock-mantle` endpoint.".into(),
            )),
        ))
    }
}
...
impl Render for ConfigurationView {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        v_flex()
            ...
            .child(
                Label::new(
                    "To use the Dez agent with Bedrock, set a custom authentication strategy in your settings file or use static credentials.",
                )
                .color(Color::Muted),
            )
            .child(
...
impl ConfigurationView {
    fn render_static_credentials_ui(&self) -> impl IntoElement {
        ...
        v_flex()
            ...
            .child(
                Label::new(format!(
                    "You can also set the {}, {} and {} environment variables (or {} for Bedrock API Key authentication) and restart Dez.",
                    ZED_BEDROCK_ACCESS_KEY_ID_VAR.name,
                    ZED_BEDROCK_SECRET_ACCESS_KEY_VAR.name,
