    ...
    fn request_layout(
        ...
        let height: Length = match self.terminal_view.read(cx).content_mode(window, cx) {
            ContentMode::Inline {
                ...
                // Prevent device-pixel snapping from shortening the requested height before the
                // terminal floors it back to a row count.
            ...
            |_, _, hitbox, window, cx| {
                ...
                let (dimensions, line_height_px) = {
                    ...
                    if matches!(self.terminal_view.read(cx).mode, TerminalMode::Standalone) {
                        ...
                        if should_anchor_to_bottom && should_bottom_anchor {
                            origin.y += padding;
                        }
                    }
