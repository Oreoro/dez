...
use task::{
    DebugRequest, DebugScenario, DezDebugConfig, LaunchRequest, SharedTaskContext, TaskContext,
    VariableName,
};
use text::Point;
use util::path;
...
async fn test_save_debug_scenario_to_file(executor: BackgroundExecutor, cx: &mut TestAppContext) {
    ...
    let debug_json_content = fs
        .load(path!("/project/.dez/debug.json").as_ref())
        .await
...
async fn test_debug_modal_subtitles_with_multiple_worktrees(
    ...
    assert_eq!(
        subtitles.as_slice(),
        [path!(".dez/debug.json"), path!(".dez/debug.json")]
    );
}
...
async fn test_dap_adapter_config_conversion_and_validation(cx: &mut TestAppContext) {
    ...
    for adapter_name in adapter_names {
        ...
        let mut adapter_specific_config = dez_config.clone();
        adapter_specific_config.adapter = adapter_name.to_string().into();

        let debug_scenario = adapter
            .config_from_dez_format(adapter_specific_config)
            .await
            .unwrap_or_else(|_| {
                panic!(
                    "Adapter {} should successfully convert from dez format",
                    adapter_name
                )
            });
