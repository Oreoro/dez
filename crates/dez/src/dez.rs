...
pub fn init(cx: &mut App) {
    ...
    .on_action(|_: &dez_actions::OpenLicenses, cx| {
        with_active_or_new_workspace(cx, |workspace, window, cx| {
            open_bundled_file(
                workspace,
    ...
    .on_action(|&dez_actions::OpenKeymapFile, cx| {
    ...
    .on_action(|_: &dez_actions::OpenDefaultKeymap, cx| {
        with_active_or_new_workspace(cx, |workspace, window, cx| {
            open_bundled_file(
                workspace,
...
fn initialize_file_watcher(window: &mut Window, cx: &mut Context<Workspace>) {
    if let Err(e) = fs::fs_watcher::global(|_| {}) {
        ...
        let message = localization::text_with_args(cx, "dez-inotify-detail", &arguments);
        let title = localization::text(cx, "dez-inotify-title");
        let troubleshoot_and_quit = localization::text(cx, "dez-troubleshoot-and-quit");
...
fn initialize_file_watcher(window: &mut Window, cx: &mut Context<Workspace>) {
    if let Err(e) = fs::fs_watcher::global(|_| {}) {
        let mut arguments = localization::FluentArgs::new();
        arguments.set("error", e.to_string());
        let message = localization::text_with_args(cx, "dez-windows-watcher-detail", &arguments);
        let title = localization::text(cx, "dez-windows-watcher-title");
        let troubleshoot_and_quit = localization::text(cx, "dez-troubleshoot-and-quit");
        let prompt = window.prompt(
            PromptLevel::Critical,
            title.as_ref(),
...
fn show_software_emulation_warning_if_needed(
    ...
    if specs.is_software_emulated && std::env::var("ZED_ALLOW_EMULATED_GPU").is_err() {
        ...
        let message = localization::text_with_args(cx, "dez-unsupported-gpu-detail", &arguments);
        let title = localization::text(cx, "dez-unsupported-gpu-title");
        let skip = localization::text(cx, "dez-skip");
        let troubleshoot_and_quit = localization::text(cx, "dez-troubleshoot-and-quit");
        let prompt = window.prompt(
            PromptLevel::Critical,
            title.as_ref(),
...
fn register_actions(
    ...
    workspace
        ...
        .register_action(|workspace, action: &dez_actions::OpenRemote, window, cx| {
        ...
        .register_action({
            ...
            move |_, action: &dez_actions::IncreaseUiFontSize, _window, cx| {
                if action.persist {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &dez_actions::DecreaseUiFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, cx| {
                        let ui_font_size = ThemeSettings::get_global(cx).ui_font_size(cx) - px(1.0);
                        let _ = settings
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &dez_actions::ResetUiFontSize, _window, cx| {
                if action.persist {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &dez_actions::IncreaseBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, cx| {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &dez_actions::DecreaseBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, cx| {
                        let buffer_font_size =
                            ThemeSettings::get_global(cx).buffer_font_size(cx) - px(1.0);
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &dez_actions::ResetBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, _| {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &dez_actions::ResetAllZoom, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, _| {
                        settings.theme.ui_font_size = None;
                        settings.theme.buffer_font_size = None;
        ...
        .register_action(
            |workspace: &mut Workspace,
             _: &dez_actions::project_panel::ToggleFocus,
             window: &mut Window,
             cx: &mut Context<Workspace>| {
        ...
        .register_action(
            |workspace: &mut Workspace,
             _: &dez_actions::agent_threads::ToggleFocus,
             window: &mut Window,
             cx: &mut Context<Workspace>| {
                    ...
                    .child(
                        v_flex()
                            ...
                            .when_some(self.commit.clone(), |this, commit| {
                                this.child(
                                    Label::new(localization::text(cx, "dez-commit"))
                                        .color(Color::Muted)
                            ...
                            .child(
                                Label::new(localization::text(cx, "dez-version"))
                                    .color(Color::Muted)
                                    .size(LabelSize::XSmall),
                            )
        ...
        if should_confirm && let Some(multi_workspace) = workspace_windows.first() {
            let answer = multi_workspace
                .update(cx, |_, window, cx| {
                    window.prompt(
                        PromptLevel::Info,
                        localization::text(cx, "dez-confirm-quit").as_ref(),
                        None,
                        &[
                            localization::text(cx, "dez-quit").as_ref(),
...
mod tests {
    ...
    async fn test_agent_threads_toggle_focus_opens_panel(cx: &mut TestAppContext) {
        ...
        cx.dispatch_action(
            ...
            dez_actions::agent_threads::ToggleFocus,
        );
        cx.run_until_parked();
    ...
    async fn test_opening_project_settings_when_excluded(cx: &mut gpui::TestAppContext) {
        ...
        eprintln!("Created project with .dez/settings.json containing UNIQUEVALUE");
        ...
        let original_settings = app_state
            .fs
            .load(Path::new("/root/.dez/settings.json"))
            .await
            .unwrap();
        ...
        // 3. Add .dez to file scan exclusions in user settings
        cx.update_global::<SettingsStore, _>(|store, cx| {
            store.update_user_settings(cx, |worktree_settings| {
                worktree_settings.project.worktree.file_scan_exclusions =
                    Some(vec![".dez".to_string()]);
            });
        });

        eprintln!("Added .dez to file_scan_exclusions in settings");
        ...
        // If .dez is not excluded, the test will fail here
        assert!(
            !has_dez_entry,
            "Test precondition failed: .dez directory should be excluded but was found in worktree"
        );
