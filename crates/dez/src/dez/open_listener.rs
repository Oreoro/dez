...
impl OpenRequest {
    ...
    pub fn parse(request: RawOpenRequest, cx: &App) -> Result<Self> {
        ...
        for url in request.urls {
            if let Some(server_name) = url.strip_prefix("dez-cli://") {
                this.kind = Some(OpenRequestKind::CliConnection(connect_to_cli(server_name)?));
            } else if let Some(action_index) = url.strip_prefix("dez-dock-action://") {
                this.kind = Some(OpenRequestKind::DockMenuAction {
                    index: action_index.parse()?,
                });
            } else if let Some(file) = url.strip_prefix("file://") {
                this.parse_file_path(file)
            } else if let Some(file) = url.strip_prefix("dez://file") {
                this.parse_file_path(file)
            } else if let Some(file) = url.strip_prefix("dez://ssh") {
                let ssh_url = "ssh:/".to_string() + file;
                this.parse_ssh_file_path(&ssh_url, cx)?
            } else if let Some(extension_id) = url.strip_prefix("dez://extension/") {
                this.kind = Some(OpenRequestKind::Extension {
                    extension_id: extension_id.to_string(),
                });
            } else if url.starts_with("dez://agent")
                || url.starts_with("dez://skill")
                || url.starts_with("dez://agent-server")
            {
                log::error!("retired agent URL ignored: {}", url);
            } else if url == "dez://" || url == "dez://open" || url == "dez://open/" {
                this.kind = Some(OpenRequestKind::FocusApp);
            } else if let Some(schema_path) = url.strip_prefix("dez://schemas/") {
                this.kind = Some(OpenRequestKind::BuiltinJsonSchema {
                    schema_path: schema_path.to_string(),
                });
            } else if url == "dez://settings" || url == "dez://settings/" {
                this.kind = Some(OpenRequestKind::Setting { setting_path: None });
            } else if let Some(setting_path) = url.strip_prefix("dez://settings/") {
                this.kind = Some(OpenRequestKind::Setting {
                    setting_path: Some(setting_path.to_string()),
                });
            } else if let Some(clone_path) = url.strip_prefix("dez://git/clone") {
                this.parse_git_clone_url(clone_path)?
            } else if let Some(commit_path) = url.strip_prefix("dez://git/commit/") {
...
pub fn listen_for_cli_connections(opener: OpenListener) -> Result<()> {
    ...
    let sock_path = paths::data_dir().join(format!("dez-{}.sock", *RELEASE_CHANNEL_NAME));
...
async fn open_local_workspace(
    ...
    // `std::env::current_dir()`, since the dez app process's cwd is typically
...
mod tests {
    use super::*;
    use crate::dez::{open_listener::open_local_workspace, tests::init_test};
    use cli::CliResponse;
    use editor::Editor;
    use futures::poll;
    ...
    fn test_retired_agent_urls_are_ignored(cx: &mut TestAppContext) {
        ...
        for url in [
            "dez://agent",
            "dez://agent/?prompt=hello",
            "dez://agent/shared/123e4567-e89b-12d3-a456-426614174000",
            "dez://skill?data=unused",
        ] {
            let request = cx.update(|cx| {
                OpenRequest::parse(
                    RawOpenRequest {
    ...
    fn test_parse_focus_app_url(cx: &mut TestAppContext) {
        ...
        for url in ["dez://", "dez://open", "dez://open/"] {
            let request = cx.update(|cx| {
                OpenRequest::parse(
                    RawOpenRequest {
                        urls: vec![url.into()],
    ...
    fn test_parse_git_commit_url(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["dez://git/commit/abc123?repo=path/to/repo".into()],
                    ..Default::default()
                },
                cx,
            )
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["dez://git/commit/def456?repo=path%20with%20spaces".into()],
                    ..Default::default()
                },
                cx,
            )
        ...
        cx.update(|cx| {
            assert!(
                OpenRequest::parse(
                    RawOpenRequest {
                        urls: vec!["dez://git/commit/abc123?repo=".into()],
                        ..Default::default()
                    },
                    cx,
                )
        ...
        let result = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["dez://git/commit/abc123?foo=bar".into()],
                    ..Default::default()
                },
                cx,
            )
    ...
    async fn test_open_workspace_with_nonexistent_files(cx: &mut TestAppContext) {
        ...
        // but tell dez to add it to the current workspace
        open_workspace_file(
            path!("/root/file6.txt"),
            workspace::OpenOptions {
        ...
        // but tell dez to NOT add it to the current workspace
        open_workspace_file(
            path!("/root/file7.txt"),
            workspace::OpenOptions {
                workspace_matching: workspace::WorkspaceMatching::None,
    ...
    async fn test_e2e_no_flags_no_windows_no_prompt(cx: &mut TestAppContext) {
        ...
        let (status, prompt_shown) = run_cli_with_dez_handler(
            cx,
            app_state,
