            ...
            0 => {
                workspace
                    .update_in(cx, |workspace, window, cx| {
                        workspace
                            .toggle_modal(window, cx, |_window, cx| InstallingdezModal::new(cx));
                    })
                    .ok();
                if let Err(error) = move_to_applications(&self.app_path, cx).await {
                    workspace
                        .update_in(cx, |workspace, _window, cx| {
                            if let Some(modal) = workspace.active_modal::<InstallingdezModal>(cx) {
...
pub struct InstallingdezModal {
    focus_handle: FocusHandle,
    finished: bool,
}

impl InstallingdezModal {
...
impl EventEmitter<DismissEvent> for InstallingdezModal {}

impl ModalView for InstallingdezModal {
...
impl Focusable for InstallingdezModal {
    fn focus_handle(&self, _cx: &App) -> FocusHandle {
        self.focus_handle.clone()
    }
}

impl Render for InstallingdezModal {
