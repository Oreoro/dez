...
const ZED_REPL_DOCUMENTATION: &str = "https://github.com/shenghsi/dez/blob/main/docs/src/repl.md";
...
impl QuickActionBar {
        ...
        let dropdown_menu = PopoverMenu::new(element_id("menu"))
            .menu(move |window, cx| {
                ...
                ContextMenu::build(window, cx, move |menu, _, cx| {
                    ...
                    .custom_entry(
                        move |_window, cx| {
                            Label::new(if has_nonempty_selection {
                                localization::text(cx, "dez-run-selection")
                            } else {
                                localization::text(cx, "dez-run-line")
                            })
                            .into_any_element()
                        },
                        {
                    ...
                    .separator()
                    .action(
                        localization::text(cx, "dez-view-sessions"),
            ...
            .trigger_with_tooltip(
                ...
                Tooltip::text(localization::text(cx, "dez-repl-menu")),
            );
    ...
    pub fn render_repl_setup(&self, language: &str, cx: &mut Context<Self>) -> Option<AnyElement> {
        let tooltip = localization::tr!(cx, "dez-setup-repl-for", language = language);
        Some(
            h_flex()
