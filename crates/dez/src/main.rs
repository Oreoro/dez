...
use crate::dez::{CrashHandler, OpenRequestKind, eager_load_active_theme_and_icon_theme};
...
fn files_not_created_on_launch(errors: HashMap<io::ErrorKind, Vec<&Path>>) {
    let message = "dez failed to launch";
...
fn fail_to_open_window(e: anyhow::Error, _cx: &mut App) {
    ...
    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    {
        ...
        _cx.spawn(async move |_cx| {
            ...
            let notification_id = "dev.dez.Oops";
            proxy
                .add_notification(
                    notification_id,
...
fn main() {
    ...
    // `dez --crash-handler` Makes dez operate in minidump crash handler mode
    ...
    if args.record_etw_trace {
        ...
        if let Err(error) =
            etw_tracing::record_etw_trace(dez_pid, &output_path, etw_socket.as_str())
        {
    ...
    // `dez --printenv` Outputs environment variables as JSON to stdout
    if args.printenv {
    ...
    if args.system_specs {
        ...
        println!("dez System Specs (from CLI):\n{}", system_specs);
        return;
    }
    ...
    let failed_single_instance_check = if *dez_env_vars::ZED_STATELESS
        || *release_channel::RELEASE_CHANNEL == ReleaseChannel::Dev
    {
        ...
        #[cfg(any(target_os = "linux", target_os = "freebsd"))]
        {
            crate::dez::listen_for_cli_connections(open_listener.clone()).is_err()
        }
        ...
        #[cfg(target_os = "macos")]
        {
            use dez::mac_only_instance::*;
            ensure_only_instance() != IsOnlyInstance::Yes
        }
    };
    if failed_single_instance_check {
        println!("dez is already running");
        return;
    }
        ...
        Some(
            app.background_executor().spawn(crashes::init(
                InitCrashHandler {
                    ...
                    dez_version: semver::Version::new(
                        app_version.major,
                        app_version.minor,
                        app_version.patch,
                    )
                    .to_string(),
                    binary: "dez".to_string(),
                ...
                |pid| paths::temp_dir().join(format!("dez-crash-handler-{pid}")),
    ...
    app.run(move |cx| {
        ...
        dez::watch_settings_files(fs.clone(), cx);
        let ui_language =
        ...
        dez::init(cx);
        #[cfg(target_os = "macos")]
        dez::move_to_applications::init(cx);
        title_bar::init(cx);
...
fn handle_open_request(request: OpenRequest, app_state: Arc<AppState>, cx: &mut App) {
        ...
        match kind {
                ...
                cx.spawn(async move |cx| {
                    ...
                    workspace.update(cx, |_, window, cx| {
                        window.dispatch_action(
                            Box::new(dez_actions::Extensions {
                ...
                workspace::with_active_or_new_workspace(cx, |_workspace, window, cx| {
                    cx.spawn_in(window, async move |workspace, cx| {
                        let res = async move {
                            ...
                            let uri = format!("dez://schemas/{}", schema_path);
            ...
            OpenRequestKind::Setting { setting_path } => {
                if let Some(setting_path) = setting_path {
                    log::warn!("dez://settings/{setting_path} opens the settings file");
                }
                cx.spawn(async move |cx| {
                    let workspace =
                        workspace::get_any_active_multi_workspace(app_state, cx.clone()).await?;
                    ...
                    workspace.update(cx, |_, window, cx| {
                        window.dispatch_action(Box::new(dez_actions::OpenSettingsFile), cx)
                    })
                })
                .detach_and_log_err(cx);
            }
            ...
            OpenRequestKind::GitCommit { sha } => {
                let base_open_options = dez::open_options_for_request(
    ...
    if let Some(connection_options) = request.remote_connection {
        ...
        let base_open_options = dez::open_options_for_request(open_behavior, &location, cx);
        cx.spawn(async move |cx| {
...
#[derive(Parser, Debug)]
#[command(name = "dez", disable_version_flag = true, max_term_width = 100)]
struct Args {
    ...
    ///
    /// URLs can either be `file://` or `dez://` scheme.
    ...
    /// On macOS, the default is `~/Library/Application Support/dez`.
    /// On Linux/FreeBSD, the default is `$XDG_DATA_HOME/dez`.
    /// On Windows, the default is `%LOCALAPPDATA%\dez`.
    ...
    /// Instructs dez to run as a dev server on this machine. (not implemented)
    #[arg(long)]
    ...
    /// prevents dez from starting, so you can't run `dez: copy system specs to
    /// clipboard`
    #[arg(long)]
    system_specs: bool,

    /// Used for recording minidumps on crashes by having dez run a separate
    /// process communicating over a socket.
    #[arg(long, hide = true)]
    crash_handler: Option<PathBuf>,

    /// Run dez in the foreground, only used on Windows, to match the behavior on macOS.
    #[arg(long)]
    #[cfg(target_os = "windows")]
    #[arg(hide = true)]
    foreground: bool,
...
async fn installation_id(db: KeyValueStore) -> Result<String> {
    if let Some(installation_id) = db.read_kvp("installation_id")? {
