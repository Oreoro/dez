...
fn main() {
    #[cfg(target_os = "linux")]
    {
        ...
        for lib in &dlopened_libs {
            if let Some(libdir) = pkg_config::get_variable(lib, "libdir").ok() {
                ...
                eprintln!("dez build.rs: {lib} not found in pkg-config's path");
            }
        }
    ...
    if cfg!(target_os = "macos") {
        ...
        // Weakly link ReplayKit to ensure dez can be used on macOS 10.15+.
...
fn prepare_app_icon_x11() {
    ...
    // name should match include_bytes! call in src/dez.rs
