//! The `language` crate provides a large chunk of dez's language-related
...
pub trait LspAdapter: 'static + Send + Sync + DynLspInstaller {
    ...
    /// provide autocompletion and diagnostics in dez setting and keybind
    /// files
    fn is_primary_dez_json_schema_adapter(&self) -> bool {
        false
    }
    ...
    /// for settings that should be persisted to dez's settings file.
    ...
    LI: LspInstaller<BinaryVersion = BinaryVersion> + LspAdapter,
{
    ...
    fn get_language_server_command(
        ...
        async move {
            ...
            //      worktree 3: no gopls found in PATH -> fallback to dez installation
            //
...
impl LanguageScope {
    ...
    ///
    /// By default, dez treats as paragraph and comment prefixes as boundaries.
    ...
    ///
    /// By default, dez treats alphanumeric characters (and '_') as word characters for
