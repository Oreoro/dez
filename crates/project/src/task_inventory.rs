...
impl<T: InventoryContents> InventoryFor<T> {
    fn worktree_scenarios(
        ...
        let has_dez_dir = worktree_dirs
            .map(|dirs| {
                dirs.keys()
                    .any(|dir| dir.file_name().is_some_and(|name| name == ".dez"))
            })
            .unwrap_or(false);
        ...
        worktree_dirs
            ...
            .filter(move |(directory, _)| {
                !(has_dez_dir && directory.file_name().is_some_and(|name| name == ".vscode"))
            })
...
pub enum TaskSourceKind {
    ...
    UserInput,
    /// Tasks from the worktree's .dez/task.json
    Worktree {
        id: WorktreeId,
        directory_in_worktree: Arc<RelPath>,
        id_base: Cow<'static, str>,
    },
    /// ~/.config/dez/task.json - like global files with task definitions, applicable to any path
    AbsPath {
        id_base: Cow<'static, str>,
        abs_path: PathBuf,
    },
...
impl Inventory {
    ...
    pub fn used_and_current_resolved_tasks(
        ...
        let worktrees_with_dez_tasks: HashSet<WorktreeId> = self
        ...
        let previously_spawned_tasks = self
            ...
            .filter(|(task_kind, _)| {
                ...
                {
                    !(worktrees_with_dez_tasks.contains(id)
