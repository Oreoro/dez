...
/// Represents the handler of a git cat-file --batch process within dez
...
impl RepositorySnapshot {
    ...
    /// created from.
    ///
    /// For example, if you had both `~/code/dez` and `~/code/worktrees/dez-2`,
    /// then `~/code/dez` is the main worktree and `~/code/worktrees/dez-2` is a linked worktree.
    ///
    ...
    pub fn access(&mut self, _cx: &App) -> oneshot::Receiver<GitAccess> {
        self.send_job("access", None, move |git_repo, _cx| async move {
            match git_repo {
                ...
                // that's running the dez remote may not own the `.git/`
...
/// - Otherwise (e.g. `dez.git` for a bare clone), `common_dir` itself is
