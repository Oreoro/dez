...
async fn test_editorconfig_support(cx: &mut gpui::TestAppContext) {
    ...
    // .editorconfig overrides .dez/settings
    ...
    // When max_line_length is "off", default to .dez/settings.json
...
async fn test_git_provider_project_setting(cx: &mut gpui::TestAppContext) {
    ...
    fs.atomic_write(
        Path::new(path!("/dir/.dez/settings.json")).to_owned(),
        "{}".into(),
    )
    .await
    .unwrap();
        ...
        vec![
            (
                TaskSourceKind::Worktree {
                    id: worktree_id,
                    directory_in_worktree: rel_path("b/.dez").into(),
                    id_base: "local worktree tasks from directory \"b/.dez\"".into()
                },
...
async fn test_invalid_local_tasks_shows_toast_with_doc_link(cx: &mut gpui::TestAppContext) {
    ...
    // We need to start with a valid `.dez/tasks.json` file as otherwise the
    ...
    // Update the `.dez/tasks.json` file with an invalid variable, so we can
    // later assert that the `Event::Toast` even is emitted.
    fs.save(
        path!("/dir/.dez/tasks.json").as_ref(),
