...
| UI/UX                  | Repaired Canvas states, pane focus, Session Rail search/start states, density and contrast work exist; the unlocked macOS screenshot exposed and drove a fix for a full-window rail overlay | Fresh corrected-artifact capture, complete shell hierarchy, outward polish, onboarding, accessibility, and state-by-state audit |
...
`dist/Superzed.app` were not opened. The first unlocked desktop capture exposed
a client-decoration bug that stretched the Session Rail over the entire
window. Commit `36d8024280` fixes the geometry and rebuilds the exact raw Dez
binary; because bare Mach-O processes are not targetable through the approved
accessibility surface, a fresh corrected-artifact capture remains open and no
broader visual claim is inferred from the build alone.
...
- [ ] Make New Window create another viewport without silently creating a
      separate application universe.
- [ ] Prove multiple viewports cannot duplicate, reorder, or garbage-collect
      durable Workspace membership accidentally. Focused source tests now
      cover identity deduplication and removal of one viewport copy; runtime
      proof remains open.
- [ ] Add focused persistence and startup-order tests for empty, unresolved,
      reordered, multi-viewport, queued-open, and failed-restore cases.
...
- [ ] Give every primary region a visible purpose, stable placement, clear
      ...
      supervision region Session Rail consistently; a rendered resize/collapse
      audit remains.
...
- [ ] Every empty state teaches one useful next step without funneling all work
      ...
      rather than the misleading caught-up state. The full state audit remains
      open.
